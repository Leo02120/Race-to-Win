...
// ===== GESTIONNAIRE DE LA PAGE SAISON F1 2025 - VERSION CORRIGÉE =====
...
class SeasonPageManager {
    constructor() {
        ...
        this.raceResults = new Map(); // Cache pour les résultats de course
        this.initializeElements();
        this.bindEvents();
    }
    ...
    async loadSeasonRaces() {
        console.log('🏁 Chargement du calendrier F1 2025...');
        ...
        try {
            ...
            const data = await response.json();
            console.log('📊 Données saison reçues:', data);
            ...
            if (!racesData) {
                throw new Error('Format de données invalide - courses manquantes');
            }

            this.races = await this.processRacesData(racesData);
            console.log(`✅ ${this.races.length} courses traitées`);
    ...
    async processRacesData(rawRaces) {
        ...
        for (const race of rawRaces) {
            ...
            if (race.schedule && race.schedule.race && race.schedule.race.date) {
                // CORRECTION : Créer la date sans décalage horaire
                raceDate = new Date(race.schedule.race.date + 'T12:00:00Z'); // Midi UTC pour éviter les décalages
            ...
            // Récupérer le vainqueur si la course est terminée
            ...
            if (status === 'completed' && !winner) {
                try {
                    ...
                    console.warn(`⚠️ Impossible de récupérer le vainqueur pour la course ${race.round}:`, error);
                }
            }
    ...
    // Nouvelle méthode pour récupérer le vainqueur d'une course
    async getRaceWinner(year, round) {
        ...
        try {
            ...
            console.warn(`⚠️ Erreur récupération résultat course ${year}/${round}:`, error);
    ...
    formatCircuitLength(length) {
        ...
        // Si c'est déjà au bon format (ex: "5.278km"), le retourner tel quel
        if (typeof length === 'string' && length.includes('.')) {
            return length;
        }

        // Si c'est un nombre ou une chaîne comme "5278km"
        const numericValue = parseFloat(length.toString().replace(/[^\d]/g, ''));

        if (isNaN(numericValue)) return 'N/A';

        // Si le nombre est supérieur à 100, on considère que c'est en mètres
        if (numericValue > 100) {
            const km = (numericValue / 1000).toFixed(3);
            return `${km}km`;
        }

        // Sinon, c'est déjà en km
    ...
    updateStats() {
        ...
        console.log(`📊 Stats: ${completed}/${total} courses terminées, ${remaining} restantes`);
    }
    ...
    renderRaces() {
        ...
        console.log(`🎯 Affichage de ${filteredRaces.length} courses (filtre: ${this.currentFilter})`);

        // Vider la grille
        this.racesGrid.innerHTML = '';

        // Créer les cartes de course
        ...
        if (filteredRaces.length === 0) {
            this.racesGrid.innerHTML = `
                <div style="grid-column: 1 / -1; text-align: center; padding: 40px; color: var(--dark-gray);">
                    ...
                    <p>Aucune course trouvée pour ce filtre</p>
                </div>
            `;
        }
    }
    ...
    createRaceCard(race) {
        ...
        // Formater la date de manière uniforme
        ...
        // Créer un contenu uniforme - toujours les mêmes éléments
        card.innerHTML = `
        ...
        // Ajouter un event listener pour les détails
        card.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('🖱️ Clic sur course:', race.name);
    ...
    formatRaceDate(date) {
        ...
        // CORRECTION: Utiliser l'heure française
        const options = { 
            day: 'numeric', 
            month: 'long',
            year: 'numeric',
    ...
    showRaceDetails(race) {
        console.log('📋 Ouverture modal pour:', race.name);
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="detail-section">
                        ...
                        <p>Longueur: ${race.circuit.length} • ${race.circuit.corners} virages</p>
                        ${race.circuit.lapRecord && race.circuit.lapRecord !== 'N/A' ? `<p>Record du tour: ${race.circuit.lapRecord}</p>` : ''}
                    </div>

                    <div class="detail-section">
                        <h3>📅 Programme (heure française)</h3>
                        ${scheduleInfo}
                    </div>
                    ...
                    <div class="detail-section">
                        ...
                        ${race.winner ? `<p>Vainqueur: 🏆 ${race.winner}</p>` : ''}
                        ${race.teamWinner ? `<p>Équipe gagnante: ${race.teamWinner}</p>` : ''}
                    </div>
                </div>
            </div>
        `;
    ...
    formatScheduleInfo(schedule) {
        if (!schedule) return '<p>Programme à confirmer</p>';
        ...
        return info || '<p>Programme à confirmer</p>';
    }
    ...
    formatScheduleDate(date, time) {
        ...
        try {
            // CORRECTION : Créer la date avec l'heure française
            let dateObj;
            if (time) {
            ...
            const options = {
                ...
                timeZone: 'Europe/Paris' // CORRECTION: Heure française
            };
    ...
    async refresh() {
        console.log('🔄 Actualisation des données de la saison...');
...
function initSeasonPage() {
    console.log('🏁 Initialisation de la page Saison...');
...
if (typeof originalShowTabForSeason === 'function') {
    window.showTab = function(tabName) {
        ...
        // Initialiser la page saison si nécessaire
        if (tabName === 'season') {
            setTimeout(() => {
                if (!seasonPageManager) {
                    initSeasonPage();
                } else {
                    // Vérifier si les données sont obsolètes (plus de 5 minutes)
                    const lastLoad = seasonPageManager.lastLoadTime || 0;
                    const now = Date.now();
                    const fiveMinutes = 5 * 60 * 1000;

                    if (now - lastLoad > fiveMinutes) {
                        console.log('🔄 Données obsolètes, rechargement...');
                        seasonPageManager.refresh();
                    }
                }
            }, 100);
...
// Initialisation automatique si l'onglet saison est déjà actif
document.addEventListener('DOMContentLoaded', function() {
...
console.log('🏁 Module Page Saison chargé (version corrigée)');
console.log('🛠️ Debug: window.seasonDebug');